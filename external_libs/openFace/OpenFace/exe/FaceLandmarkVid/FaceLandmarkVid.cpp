...
int main(int argc, char **argv)
{
    ...
    int gaze_socket = socket(AF_INET, SOCK_STREAM, 0);
    server.sin_family = AF_INET;
    server.sin_port = htons(5005);
    server.sin_addr.s_addr = inet_addr("127.0.0.1");

    if (connect(gaze_socket, (struct sockaddr*)&server, sizeof(server)) < 0) {
        std::cerr << "Connection to Python server failed!" << std::endl;
        return 1;
    }
	...
	// Utilities::Visualizer visualizer(true, false, false, false); // TEMPORARILY DISABLED
	...
	while (true) // this is not a for loop as we might also be reading from a webcam
	{
		...
		while (!rgb_image.empty()) // this is not a for loop as we might also be reading from a webcam
		{
			...
			std::ostringstream gaze_stream;

			// gaze_stream << "{";

			// Convert gaze direction to angles
			cv::Vec2f gaze_angles = GazeAnalysis::GetGazeAngle(gazeDirection0, gazeDirection1);

			// Format as JSON-like string
			gaze_stream << "{"
						<< "\"yaw\": " << gaze_angles[0] << ", "
						<< "\"pitch\": " << gaze_angles[1] << ", "
						<< "\"gaze_left\": {\"x\": " << gazeDirection0.x << ", \"y\": " << gazeDirection0.y << ", \"z\": " << gazeDirection0.z << "}, "
						<< "\"gaze_right\": {\"x\": " << gazeDirection1.x << ", \"y\": " << gazeDirection1.y << ", \"z\": " << gazeDirection1.z << "}, "
						<< "\"head_pose\": {"
						<< "\"tx\": " << pose_estimate[0] << ", \"ty\": " << pose_estimate[1] << ", \"tz\": " << pose_estimate[2] << ", "
						<< "\"roll\": " << pose_estimate[3] << ", \"pitch\": " << pose_estimate[4] << ", \"yaw\": " << pose_estimate[5]
						<< "}"
						<< "}\n";



			std::string data = gaze_stream.str();
			ssize_t sent_bytes = send(gaze_socket, data.c_str(), data.length(), 0);
			if (sent_bytes < 0) {
	...
	close(gaze_socket); // CLOSE SOCKET BEFORE EXIT
